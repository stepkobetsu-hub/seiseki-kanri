...
const AUTH_API = 'https://script.google.com/macros/s/AKfycbypkUc0MqZ07E7pZRglNPeRM56WbCcuWaLpRzi9bVFcPklHDxaaLC7GfzG6ozTGCbEX/exec';
...
function loginEmployeeMaster(code, password) {
  ...
  return {success:true, name:result.name || '', permissionLevel:level, token:result.systemPortalSessionToken || '', expiresAt:result.systemPortalExpiresAt || ''};
}

function verifyToken_(token) {
  const result = callAuth_({action:'verifySystemPortal', systemPortalSessionToken:String(token || '')});
  const level = String(result.permissionLevel || '');
  if (!result.success || !ALLOWED_PERMISSIONS.includes(level)) throw new Error('ログインの有効期限が切れました。もう一度ログインしてください。');
  return result;
}

function callAuth_(payload) {
  const res = UrlFetchApp.fetch(AUTH_API, {method:'post', contentType:'text/plain', payload:JSON.stringify(payload), muteHttpExceptions:true});
